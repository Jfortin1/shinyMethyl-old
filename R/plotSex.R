	...
	plotPredictedGender <- function(cnQuantiles,cutoff =(-0.3), color = NULL, legend=TRUE, bty="o"){
	    X <- cnQuantiles$X
		Y <- cnQuantiles$Y
		med <- floor(nrow(X)/2)
	...
	plotDiscrepancyGenders <- function(cutoff = (-0.3), covariates, cnQuantiles, bty="o"){
		
		predictedGender <- returnPredictedGender(cutoff = cutoff, cnQuantiles = cnQuantiles)
