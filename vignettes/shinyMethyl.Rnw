...
Please see the different figures at the end of the vignette that explain each of the panels.
...
\caption{\textbf{Comparison of raw and normalized data} As discussed in the Advanced option, normalized data can be added as well to the visualization interface. In the present plot, the above densities represent the M-value densities after functional normalization as described in \citep{funnorm}.}
\label{panel1}
\end{center}
\end{figure}
...
\caption{\textbf{Visualization of batch effects in the TCGA HNSCC dataset} Densities of the M-values
  for Type I green probes before (a) and after (b) functional normalization \citep{funnorm} as presented in the
